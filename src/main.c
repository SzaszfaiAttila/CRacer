...
static int         play_sel=0;   /* 0=START  1=BACK in play-select screen  */
...
static Mesh   g_hitbox_mesh;  /* unit box centred at origin, rebuilt at startup */

/* ── OBJ bike models ─────────────────────────────────────────────────────── */
static ObjModel g_bike_player;       /* bike-white.obj              */
static ObjModel g_bike_ai;           /* bike-orange.obj             */
static ObjModel g_bike_player_refl;  /* bike-white-reflection.obj   */
static ObjModel g_bike_ai_refl;      /* bike-orange-reflection.obj  */

/* ── Bike model transform: tweak these to fit the OBJ coordinate system ──── */
/* Scale: 0.10 = 10 % of original size                                        */
/* Rot Y : rotation applied BEFORE the bike faces its travel direction         */
/*   0.0      = model faces +Z (OBJ default "forward")                        */
/*  -M_PI/2  = 90° left  (model originally faces +X, needs to face -Z)       */
/* Scale / orientation / trail-alignment — tune here */
#define BIKE_MODEL_SCALE    0.025f
#define BIKE_MODEL_ROT_Y    ((float)M_PI * 0.5f)      /* 90 degrees right */
/* Push the visual model forward so it sits in front of the trail origin.
   Positive = forward in the bike's travel direction (world space). */
#define BIKE_MODEL_FORWARD  1.5f
/* Push model up so wheels sit on the arena surface, not in it */
#define BIKE_MODEL_HEIGHT_OFFSET  0.7f

/* Returns best available mesh/model for each role (falls back to box mesh) */
static int has_player     (void){ return g_bike_player.mesh.vert_count     > 0; }
static int has_ai         (void){ return g_bike_ai.mesh.vert_count         > 0; }
static int has_player_refl(void){ return g_bike_player_refl.mesh.vert_count> 0; }
static int has_ai_refl    (void){ return g_bike_ai_refl.mesh.vert_count    > 0; }

static Mesh *bike_mesh_player     (void){ return has_player()      ? &g_bike_player.mesh      : &g_scene.bike_mesh; }
static Mesh *bike_mesh_ai         (void){ return has_ai()          ? &g_bike_ai.mesh          : &g_scene.bike_mesh; }
static Mesh *bike_mesh_player_refl(void){ return has_player_refl() ? &g_bike_player_refl.mesh : bike_mesh_player(); }
static Mesh *bike_mesh_ai_refl    (void){ return has_ai_refl()     ? &g_bike_ai_refl.mesh     : bike_mesh_ai();     }

/* Build a bike model matrix: translate(+forward offset) → dir → model-rot → scale
   forward_off: extra world-space offset along the bike's travel direction (for
   positioning the OBJ model so the trail starts from behind the rear axle).
   reflect_y  : +1.0 normal pass, -1.0 Y-mirrored reflection pass.           */
static void bike_model_mat(mat4 out, float wx, float wy, float wz,
                            float dir_angle, int use_obj,
                            float reflect_y) {
    glm_mat4_identity(out);

    /* For OBJ models, push the visual centre forward so the trail starts
       behind the bike rather than at its centre.                              */
    float fwd = (use_obj) ? BIKE_MODEL_FORWARD : 0.0f;
    /* dir_angle encodes travel direction; forward vector is obtained by
       rotating the -Z axis by dir_angle around Y.                            */
    float fx = -sinf(dir_angle) * fwd;   /* -Z rotated by angle = this */
    float fz = -cosf(dir_angle) * fwd;

    float oy = (use_obj) ? BIKE_MODEL_HEIGHT_OFFSET : 0.0f;
    vec3 pos = {wx + fx, wy + oy, wz + fz};
    ...
    if (use_obj) {
        ...
        vec3 sc = {BIKE_MODEL_SCALE,
                   BIKE_MODEL_SCALE * reflect_y,
                   BIKE_MODEL_SCALE};
        glm_scale(out, sc);
    } else {
        if (reflect_y < 0.0f) {
            vec3 sc = {1.0f,-1.0f,1.0f}; glm_scale(out, sc);
        }
    }
}
...
/* Button — selected forces keyboard-nav highlight regardless of mouse position */
static int do_button(float x, float y, float w, float h,
    ...
    /* Dark navy fill inside borders */
    ui_rect(&ui, x+2, y+2, w-4, h-4, 0.0f, 0.03f, 0.09f, alpha*0.58f);

    /* Glowing border */
    float bdr = hov ? 1.0f : 0.38f;
    float gc  = hov ? 0.60f : 0.0f;
    ...
    float tr = hov ? 0.10f : 0.0f;
    float tg = hov ? 1.0f  : 0.65f;
    float tb = hov ? 1.0f  : 0.80f;
    ui_text(&ui, label, tx, ty, fsz, tr, tg, tb, alpha);

    return hov && g_lmb_clicked;
}

/* Slider — val in [0, val_max]; percentage displays val*100 */
...
static void render_main_menu(void){
    ...
    ui_text(&ui, "LIGHT CYCLE ARENA", lx, by+SZ_HUGE*1.3f, SZ_SMALL,
            0.18f, 0.52f, 0.78f, alpha*0.65f);
...
static void render_play_select(void){
    ...
    /* Arrow buttons for count — never keyboard-selected */
    ...
    /* START and BACK with keyboard navigation highlight */
    if(do_button(lx,sy2,bw,bh,"START",SZ_MEDIUM,1.0f,(play_sel==0)))
        enter_state(STATE_COUNTDOWN);
    if(do_button(lx,sy2+bh*1.5f,bw,bh,"BACK",SZ_MEDIUM,1.0f,(play_sel==1)))
        enter_state(STATE_MAIN_MENU);

    /* Mouse hover updates play_sel */
...
static void render_settings(void){
    ...
    ui_rect(&ui, lx, ry, sw2, 2.0f, 0, 0.30f, 0.50f, 0.45f);
    ry += SZ_SMALL*0.8f;
    ...
    /* Brightness  0–1.5 → 0–150%;  100% sits exactly at 2/3 of track */
    ...
    static const char *ctrl[][2]={
        ...
        {"Left Shift",     "Dash  (1.5× speed, 3 s, 6 s cooldown)"},
    ...
    if(do_button(lx,ry,bw,bh,"BACK",SZ_MEDIUM,1.0f,0))
        enter_state(settings_return);
}
...
/* Inline helpers for new shader uniforms */
static void sh_opaque(void){
    /* Fully opaque — both alpha uniforms = 1.0 */
...
static void sh_defaults(void){
    /* Reset all optional shader uniforms to safe defaults */
    shader_set_float(sh,"uAlphaTop",   1.0f);
    shader_set_float(sh,"uAlphaSide",  1.0f);
...
static void render_3d(int W, int H){
    ...
    shader_set_vec3(sh,"lightColor",0.55f*g_brightness,0.65f*g_brightness,0.90f*g_brightness);
    ...
    shader_set_mat4(sh,"model",(float*)model);
    draw_emissive(0.96f,0.98f,0.88f,2.8f,2.9f,2.4f);
    mesh_draw(&g_scene.moon);
    glm_mat4_identity(model); shader_set_mat4(sh,"model",(float*)model);

    /* ── Opaque scene — specular values per material ──────────────────────
       Arena floor: plastic-glass (high specular, very tight highlight)      */
    sh_specular(0.88f, 220.0f);
    draw_obj(0.06f,0.07f,0.11f);
    mesh_draw(&g_scene.arena);

    sh_specular(0.45f, 64.0f);
    draw_emissive(0.2f,0.5f,0.5f,0.0f,0.14f,0.20f);
    mesh_draw(&g_scene.arena_acc);

    sh_specular(0.22f, 36.0f);
    draw_obj(0.10f,0.10f,0.14f);
    mesh_draw(&g_scene.pillars);

    sh_specular(0.32f, 44.0f);
    draw_emissive(0.9f,0.95f,1.0f,0.2f,0.25f,0.35f);
    mesh_draw(&g_scene.pillar_acc);

    /* ── Opaque bikes — OBJ with MTL colours, two-sided lighting ─────────────
       Two-sided lighting (uTwoSided=1) makes both polygon faces contribute
       diffuse light, which corrects OBJ files whose normals happen to point
       inward (a common Blender export artefact depending on scale/origin).   */
    ...
    if(bike.alive||(bike.falling&&bike.y>ARENA_BASE+4.0f)){
        mat4 bm; bike_model_mat(bm, bike.x,bike.y,bike.z,
                                DANGLE[bike.dir], has_player(), 1.0f);
        shader_set_mat4(sh,"model",(float*)bm);
        if (has_player()) {
            ObjModel *m = &g_bike_player;
            /* Clamp Ns to a reasonable specular range */
            float ns_clamped = fmaxf(fminf(m->ns, 128.0f), 8.0f);
            sh_specular(ns_clamped / 200.0f, ns_clamped);
            /* Pure MTL diffuse colour — no team-colour override so the
               model looks exactly as it did in Blender                      */
            shader_set_vec3 (sh,"objectColor", m->kd_r, m->kd_g, m->kd_b);
            /* Small emissive so the bike rim-lights in the dark scene       */
            shader_set_vec3 (sh,"emissive",
                             0.06f*m->kd_r*g_brightness,
                             0.06f*m->kd_g*g_brightness,
                             0.06f*m->kd_b*g_brightness);
            shader_set_float(sh,"ambientStr", 0.32f*g_brightness);
        } else {
            shader_set_float(sh,"ambientStr", 0.12f*g_brightness);
    ...
    for(int i=0;i<MAX_AI;i++){
        ...
        if(i<ai_limit && (ab->alive||(ab->falling&&ab->y>ARENA_BASE+4.0f))){
            mat4 am; bike_model_mat(am, ab->x,ab->y,ab->z,
                                    DANGLE[ab->dir], has_ai(), 1.0f);
            shader_set_mat4(sh,"model",(float*)am);
            if (has_ai()) {
                ObjModel *m = &g_bike_ai;
                float ns_clamped = fmaxf(fminf(m->ns, 128.0f), 8.0f);
                sh_specular(ns_clamped / 200.0f, ns_clamped);
                shader_set_vec3 (sh,"objectColor", m->kd_r, m->kd_g, m->kd_b);
                shader_set_vec3 (sh,"emissive",
    ...
    /* Restore non-bike shader state */
    sh_specular(0.0f, 32.0f);
    shader_set_float(sh,"uTwoSided", 0.0f);

    /* ── Reflections: blurred Y-mirror of bikes + trails ─────────────────────
       "Blur" is approximated by rendering each element BLUR_SAMPLES times with
       small world-space jitter on X and Z, each at 1/BLUR_SAMPLES of the
       target alpha.  The additive blend accumulates the spread into a soft
       glow that reads as a blurred reflection on the glossy floor.
       BLUR_R controls the blur radius in world units.                         */
#define BLUR_SAMPLES  7
#define BLUR_R        0.55f

    /* Jitter offsets: centre + 6 around a circle */
    ...
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE);   /* additive — neon glow on floor */
    glDepthMask(GL_FALSE);
    glDepthFunc(GL_ALWAYS);

    /* Per-sample alpha: centre sample gets double weight so the reflection
       has a bright core surrounded by a soft fringe.                         */
    float base_refl_alpha = 0.18f;
    float samp_alpha_centre = base_refl_alpha * 2.0f / (float)(BLUR_SAMPLES + 1);
    float samp_alpha_outer  = base_refl_alpha * 1.0f / (float)(BLUR_SAMPLES + 1);

    for (int samp = 0; samp < BLUR_SAMPLES; samp++) {
        float jx = JITTER_X[samp];
        float jz = JITTER_Z[samp];
        float samp_alpha = (samp == 0) ? samp_alpha_centre : samp_alpha_outer;
        shader_set_float(sh,"uAlphaMult", samp_alpha);

        /* Reflected player */
        if(bike.alive||(bike.falling&&bike.y>ARENA_BASE+4.0f)){
            float ry = 2.0f*ARENA_TOP - bike.y;
            mat4 rm; bike_model_mat(rm, bike.x+jx, ry, bike.z+jz,
                                    DANGLE[bike.dir], has_player_refl(), -1.0f);
            shader_set_mat4(sh,"model",(float*)rm);
            if (has_player_refl()) {
                ObjModel *m = &g_bike_player_refl;
                shader_set_vec3(sh,"objectColor", m->kd_r, m->kd_g, m->kd_b);
                shader_set_vec3(sh,"emissive",
                                0.30f*m->kd_r*g_brightness,
                                0.30f*m->kd_g*g_brightness,
                                0.30f*m->kd_b*g_brightness);
                shader_set_float(sh,"ambientStr", 0.18f*g_brightness);
            } else {
        ...
        for(int i=0;i<ai_limit;i++){
            ...
            if(ab->alive||(ab->falling&&ab->y>ARENA_BASE+4.0f)){
                ...
                mat4 rm; bike_model_mat(rm, ab->x+jx, ry, ab->z+jz,
                                        DANGLE[ab->dir], has_ai_refl(), -1.0f);
                shader_set_mat4(sh,"model",(float*)rm);
                if (has_ai_refl()) {
                    ObjModel *m = &g_bike_ai_refl;
                    shader_set_vec3(sh,"objectColor", m->kd_r, m->kd_g, m->kd_b);
                    shader_set_vec3(sh,"emissive",
                                    0.30f*m->kd_r*g_brightness,
                                    0.30f*m->kd_g*g_brightness,
        ...
        /* Reflected trails — also jittered */
        {
            ...
            /* Trails are purely emissive in the reflection — no object colour */
            shader_set_float(sh,"ambientStr", 0.0f);
            float trail_samp = samp_alpha * 0.75f;
            shader_set_float(sh,"uAlphaMult", trail_samp);

            shader_set_vec3(sh,"objectColor",0,0,0);
            shader_set_vec3(sh,"emissive",
                            0.28f*g_brightness,0.28f*g_brightness,0.28f*g_brightness);
            dynmesh_draw(&player_trail);

            for(int i=0;i<ai_limit;i++){
                shader_set_vec3(sh,"emissive",
                                0.30f*g_brightness,0.24f*g_brightness,0.0f);
                dynmesh_draw(&ai_trail_meshes[i]);
            }
            /* Restore per-sample alpha for bikes in next iteration */
            shader_set_float(sh,"uAlphaMult", samp_alpha);
        }
    } /* end blur loop */

    /* Restore all depth / blend / uniform state */
    ...
    glm_mat4_identity(model); shader_set_mat4(sh,"model",(float*)model);
        ...
        /* ── Transparent trails (normal, src-alpha blend, after opaque+refl) ─── */
    glm_mat4_identity(model); shader_set_mat4(sh,"model",(float*)model);
    sh_specular(0.0f, 32.0f);
    ...
    draw_emissive(1.0f,1.0f,1.0f,0.3f,0.3f,0.3f);
    dynmesh_draw(&player_trail);

    for(int i=0;i<MAX_AI;i++){
        if(i<ai_limit){
            draw_emissive(1.0f,0.95f,0.2f,0.35f,0.28f,0.0f);
            dynmesh_draw(&ai_trail_meshes[i]);
        }
    }
...
#if SHOW_HITBOX
    /* ── Debug hitbox — wireframe red box at each live bike position ─────────
       Drawn over everything (no depth test) so it's always visible even when
       the camera is inside or below the arena surface.
       The box dimensions and offsets are all in config.h:
         HITBOX_HALF_W / HITBOX_HALF_L / HITBOX_HALF_H   — extents
         HITBOX_OFFSET_X / Y / Z                         — local-space shift
       The box is aligned to the bike's travel direction (same rotation as the
       trail), NOT to the OBJ model orientation, so it shows the actual
       gameplay collision zone rather than the visual model bounds.            */
    {
        shader_use(sh);
        sh_defaults();
        /* Pure red emissive — fog and lighting don't affect the debug display */
        shader_set_vec3(sh, "objectColor", 0.0f, 0.0f, 0.0f);
        shader_set_vec3(sh, "emissive",    1.0f, 0.08f, 0.08f);
        shader_set_float(sh,"ambientStr",  0.0f);
        shader_set_float(sh,"fogDensity",  0.0f);   /* no fog on debug overlay */
        ...
        glDepthFunc(GL_ALWAYS);   /* always on top */
        glDepthMask(GL_FALSE);

        /* Helper lambda-style: draw hitbox at (bx,by,bz) facing dir_angle */
        /* Player bike */
        if (bike.alive || (bike.falling && bike.y > ARENA_BASE + 4.0f)) {
            mat4 hm; glm_mat4_identity(hm);
            vec3 hp = {bike.x, bike.y, bike.z}; glm_translate(hm, hp);
            vec3 hy = {0,1,0};
            glm_rotate(hm, DANGLE[bike.dir], hy);
            shader_set_mat4(sh, "model", (float*)hm);
            mesh_draw(&g_hitbox_mesh);
        }
        /* AI bikes */
        for (int i = 0; i < ai_limit; i++) {
            Bike *ab = &ai_bikes[i].bike;
            if (ab->alive || (ab->falling && ab->y > ARENA_BASE + 4.0f)) {
                ...
                vec3 hy = {0,1,0};
                glm_rotate(hm, DANGLE[ab->dir], hy);
                shader_set_mat4(sh, "model", (float*)hm);
                mesh_draw(&g_hitbox_mesh);
            }
        }
...
int main(void){
    ...
    /* ── Debug hitbox mesh (unit box; scale applied per-bike in render) ── */
    {
        float hb[216];
        add_box(hb, 0,
                -HITBOX_HALF_W + HITBOX_OFFSET_X,
                 HITBOX_OFFSET_Y,
                -HITBOX_HALF_L + HITBOX_OFFSET_Z,
                 HITBOX_HALF_W + HITBOX_OFFSET_X,
                 HITBOX_HALF_H * 2.0f + HITBOX_OFFSET_Y,
                 HITBOX_HALF_L + HITBOX_OFFSET_Z);
        g_hitbox_mesh = mesh_create(hb, 216);
    }

    /* ── Load OBJ bike models ──────────────────────────────────────────── */
    g_bike_player      = obj_load("../assets/models/bikes/bike-white.obj");
    g_bike_ai          = obj_load("../assets/models/bikes/bike-orange.obj");
    g_bike_player_refl = obj_load("../assets/models/bikes/bike-white-reflection.obj");
    g_bike_ai_refl     = obj_load("../assets/models/bikes/bike-orange-reflection.obj");
    ...
    while(!glfwWindowShouldClose(g_win)){
        ...
        case STATE_PLAY_SELECT:
            ...
            if(nav_up||nav_dn) play_sel=1-play_sel;  /* toggle between START and BACK */
            if(nav_sel){
                if(play_sel==0) enter_state(STATE_COUNTDOWN);
                else            enter_state(STATE_MAIN_MENU);
            }
            break;
        ...
        if(game_active){
            ...
            /* death explosions — pass actual speed */
            ...
            /* Emit dash sparks for player */
            if(bike.alive && !bike.falling){
                int rl = (bike.dash_timer>0.0f)?2 : (bike.dash_cooldown<=0.0f?1:0);
                particles_emit_dash(bike.x, bike.y, bike.z, bike.dir,
                                    0.2f, 1.0f, 1.0f,   /* cyan */
                                    rl, game_dt);
            }
            /* Emit dash sparks for active AIs */
            for(int i=0;i<g_n_ai;i++){
                Bike *ab=&ai_bikes[i].bike;
                if(ab->alive && !ab->falling){
                    int rl=(ab->dash_timer>0.0f)?2:(ab->dash_cooldown<=0.0f?1:0);
                    particles_emit_dash(ab->x, ab->y, ab->z, ab->dir,
                                        1.0f, 0.6f, 0.05f,   /* orange */
                                        rl, game_dt);
                }
            }
        ...
        /* Player trail: empty in menu context to avoid ghost geometry */
        ...
        /* AI trails: only build for active slots; clear inactive */
