...
int main(void) {
    ...
    while (!glfwWindowShouldClose(win)){
        ...
        // Scene
        draw_obj     (sh,0.02f,0.04f,0.12f);                 mesh_draw(&scene.sea);
        ...
        glm_mat4_identity(model);
        vec3 bpos={bike.x,ARENA_TOP,bike.z};
        glm_translate(model,bpos);
        vec3 yaxis={0,1,0};
        glm_rotate(model,DANGLE[bike.dir],yaxis);
        shader_set_mat4(sh,"model",(float*)model);
        if (bike.alive) draw_emissive(sh,0.6f,1.0f,1.0f,0.1f,0.6f,0.7f);
        else            draw_emissive(sh,0.5f,0.1f,0.1f,0.3f,0.0f,0.0f);
        ...
        for (int i=0;i<N_AI;i++){
            ...
            glm_mat4_identity(model);
            vec3 apos={ai_bikes[i].bike.x,ARENA_TOP,ai_bikes[i].bike.z};
            glm_translate(model,apos);
            glm_rotate(model,DANGLE[ai_bikes[i].bike.dir],yaxis);
            shader_set_mat4(sh,"model",(float*)model);
            if (ai_bikes[i].bike.alive)
                draw_emissive(sh,1.0f,0.5f,0.05f,0.5f,0.18f,0.0f);   // orange
            else
                draw_emissive(sh,0.3f,0.15f,0.05f,0.15f,0.05f,0.0f); // dark
            mesh_draw(&scene.bike_mesh);
        }

        // Reset model for particles
        glm_mat4_identity(model);
        shader_set_mat4(sh,"model",(float*)model);
